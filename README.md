...
7. 这个二维码**必须用你的手机微信摄像头扫码**，不能保存到相册扫码。你可以截图二维码后发送到电脑/另一部手机，然后在电脑/另一部手机上打开图片，用当前手机扫码。
8. 扫码后在手机上确认登录，然后返回到浏览器的微信登录页面。
9. 登录成功后，进入「关注」页面，搜索公众号名称来关注。新关注的公众号归入”默认”分组，你可以新建分组，并把关注的公众号归入新的分组。
10. 回到「信息流」页面，选择分组标签查看文章。
...
- 微信登录会话的有效期由微信服务器控制，通常是 4 天左右
- 当会话过期时会出现无法获取新文章的现象，且「我的」页面会有提醒，需要在该页面重新扫码登录
